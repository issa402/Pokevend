...
type InventoryService struct{ inv store.InventoryStore }

func NewInventoryService(inv store.InventoryStore) *InventoryService {
...
func (s *InventoryService) ImportCSV(ctx context.Context, userID string, r io.Reader) (int, error) {
	...
	for _, row := range records[1:] { // skip header row
		...
		if v := safeGet(row, 1); v != "" { item.SetName = &v }
		if v := safeGet(row, 2); v != "" { item.CardNumber = &v }
		if v := safeGet(row, 3); v != "" { item.Condition = &v }
